...
validKeys = {"label": "Meaningful object name",
			 "points": "A list of [x,y] coordinates of the vertices of the path. Do not duplicate the first point at the end",
...
class Polygon(cncObject):
	def __init__(self, parent, params):
		...
		if valid is not None:
			self.createErrors(["Parameter errors for Polygon:"] + valid)

		if "label" not in self.params:
			self.params["label"] = "Polygon"

		if "points" not in self.params:
			self.params["points"] = [[0, 0], [0, 10], [10, 10], [10, 0]]
	...
	def getParameters(self):
		...
		props = {
			...
			"points": {"label": "Data Points", "dataType": "point2dlist", "value": self.params["points"], "minvals": 3},
	...
	def render(self, cnc, reporter):
		...
		for i in range(passes):
			...
			rendering.extend(cnc.cutXY(points[0]))
